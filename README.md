...
- 발광 아이템 액자로 캔버스, 갤러리, 팔레트, 조작판의 조명 차이 최소화 (월드 전시는 기본 아이템 액자 사용)
...
- 설치된 팔레트는 주변 플레이어에게도 보이지만 소유자만 조작할 수 있습니다.
...
그림이 있는 캔버스를 사용하다 접속을 종료하면 그림은 갤러리에 등록되지 않고 플레이어별 자유 모드 임시본으로 저장되며, 설치한 캔버스와 작업 UI는 월드에서 철거됩니다. 다음 접속 때 임시본 안내가 표시되고, 새 그림판을 선택하면 기존 크기로 캔버스를 설치한 뒤 전체 레이어와 편집 상태를 복원합니다.

완성 저장이 성공하면 자유 모드 임시본은 삭제됩니다. 새 그림을 시작하려면 메인 패널의 캔버스 제거를 두 번 누르거나 `/paint remove`로 임시본을 삭제합니다. 빈 캔버스로 접속을 종료한 경우에는 임시본을 남기지 않습니다.
...
수동 모드에서는 관리자가 미리 캔버스, 갤러리, 조작판, 팔레트 위치를 지정합니다. 네 위치가 모두 등록되어야 자리를 사용할 수 있으며, 일반 플레이어는 관리자가 설치한 자리에서만 그림을 그리고 저장합니다.

수동 모드에서는 관리자도 일반 `/paint` 메뉴와 자유 캔버스 명령을 사용할 수 없습니다. 관리자는 `/paint station ...`, `/paint gallery <플레이어명>`, `/paint exhibits ...` 명령만 사용할 수 있으며, 실제 그림 작업은 지정된 그림판을 우클릭해서 시작합니다.

이 때, 캔버스 위치 정보는 `plugins/Paint/manual-stations.yml`에 저장됩니다.

수동 조작판에는 완성 저장, 임시저장, 작업 종료 버튼이 표시됩니다. 임시저장 버튼을 누르거나 그림을 그리던 중 접속을 종료하면 그림은 갤러리에 등록되지 않고 해당 자리와 플레이어의 임시본으로 보관되며, 월드의 캔버스는 빈 화면으로 돌아갑니다. 같은 플레이어가 같은 캔버스를 다시 우클릭하면 레이어를 포함한 임시본을 불러옵니다. 완성 저장이나 작업 종료가 완료되면 해당 임시본은 삭제됩니다.

| 수동 조작판 버튼 | 아이콘 | 동작 |
| --- | --- | --- |
| 완성 저장 | 플로피디스크 | 그림 이름을 입력해 갤러리에 저장하고 작업을 종료합니다. 기존 임시본은 삭제됩니다. |
| 임시저장 | 뒤로가기 화살표 | 갤러리에는 등록하지 않고 자리와 플레이어별 임시본으로 보관한 뒤 작업을 종료합니다. |
| 작업 종료 | 휴지통 | 두 번 눌러 확정하면 현재 그림과 임시본을 삭제하고 작업을 종료합니다. |

수동 자리를 사용하는 동안 팔레트 도구로 팔레트 보드를 제거하거나 다른 위치에 다시 설치할 수 없습니다. 색상과 브러시 크기는 관리자가 지정한 고정 팔레트에서 그대로 선택할 수 있습니다.

## 명령어

`free-mode: true`인 자유 모드에서는 일반 플레이어가 `/paint` 메뉴와 자유 캔버스 명령을 사용할 수 있습니다. `free-mode: false`인 수동 모드에서는 관리자도 일반 `/paint` 메뉴와 자유 캔버스 명령을 사용할 수 없으며, 관리자용 자리·갤러리·전시 명령만 사용할 수 있습니다.

| 명령어 | 사용 모드 | 설명 |
| --- | --- | --- |
| `/paint` | 자유 모드 | Paint 메인 메뉴를 엽니다. 이미 열려 있으면 현재 위치로 다시 소환합니다. |
| `/paint new [너비] [높이]` | 자유 모드 | 새 캔버스 설치 프리뷰를 시작합니다. |
| `/paint clear` | 자유 모드 | 현재 캔버스를 비웁니다. |
| `/paint remove` | 자유 모드 | 현재 캔버스와 자유 모드 임시본을 제거합니다. |
| `/paint save [이름]` | 자유 모드 | 현재 캔버스를 PNG 그림으로 저장합니다. |
| `/paint list` | 자유 모드 | 저장한 그림 목록을 엽니다. |
| `/paint show` | 자유 모드 | 저장한 그림 전시 흐름을 시작합니다. |
| `/paint gallery <플레이어명>` | 전체 모드 · 관리자 | 해당 플레이어의 갤러리를 엽니다. |
| `/paint brush <크기>` | 자유 모드 | 브러시 반지름을 변경합니다. |
| `/paint color <색상>` | 자유 모드 | 등록된 팔레트 색상으로 현재 색상을 바꿉니다. |
| `/paint exhibits reload` | 전체 모드 · 관리자 | 전시품 데이터를 다시 불러옵니다. |
| `/paint exhibits remove` | 전체 모드 · 관리자 | 바라보고 있는 전시품을 제거하는 모드에 들어갑니다. |
...
| `/paint station palette <번호>` | 해당 번호의 팔레트 설치 프리뷰를 시작합니다. |
...
| `manual-stations.yml` | 수동 자리 모드의 캔버스, 갤러리, 조작판, 팔레트 위치입니다. |
| `free-drafts` | 자유 모드 플레이어별 임시 캔버스 레이어 데이터입니다. 갤러리에는 표시되지 않습니다. |
| `manual-drafts` | 수동 자리와 플레이어별 임시 캔버스 레이어 데이터입니다. 갤러리에는 표시되지 않습니다. |
