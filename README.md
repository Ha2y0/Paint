...
수동 모드에서 일반 플레이어는 지정된 그림판을 우클릭해서 작업을 시작해야 합니다. `paint.admin` 권한이 있는 관리자는 `/paint` 전체 메뉴와 자유 캔버스 기능을 포함한 모든 관리자 명령을 사용할 수 있습니다.
...
`free-mode: true`인 자유 모드에서는 일반 플레이어가 `/paint` 메뉴와 자유 캔버스 명령을 사용할 수 있습니다. `free-mode: false`인 수동 모드에서는 일반 플레이어의 `/paint` 명령이 차단되지만, `paint.admin` 권한이 있는 관리자는 전체 메뉴와 모든 관리자 명령을 사용할 수 있습니다.
