...
| 명령어 | 설명 |
| --- | --- |
| `/paint station canvas <번호> <너비> <높이>` | 바라보는 블록을 해당 번호의 캔버스 위치로 저장합니다. |
| `/paint station gallery <번호>` | 현재 위치 기준으로 해당 번호의 갤러리 위치를 저장합니다. |
| `/paint station control <번호> [horizontal|vertical]` | 바라보는 벽에 조작판 위치와 배치를 저장합니다. |
| `/paint station list` | 등록된 수동 자리 목록을 봅니다. |
| `/paint station remove <번호>` | 해당 번호의 수동 자리를 삭제합니다. |
...
| 설정 | 기본값 | 설명                                   |
| --- | --- |--------------------------------------|
| `map-render.rgb-mode` | `true` | 지도 렌더링에서 RGB 색상 표현을 사용합니다.           |
